...
mydb=mysql.connector.connect(user='root',password='Mani@2002',host='localhost',db='snmprjdb')
...
def download_file(fid):
    if session.get('user'):
        cursor = mydb.cursor(buffered=True)
        cursor.execute(
            'SELECT fid, file_name, file_data FROM filedata WHERE fid = %s AND added_by = %s',
        ...
        if fdata:
            ...
            return send_file(
                data,
                download_name=fdata[1],
                mimetype=mime_type or 'application/octet-stream',
                as_attachment=True  # Ensures the file is downloaded
            )
        else:
...
@app.route('/delete_file/<fid>', methods=['GET'])
def delete_file(fid):
    if session.get('user'):
        cursor = mydb.cursor()
        # Check if file exists and belongs to the user
        cursor.execute('SELECT fid FROM filedata WHERE fid = %s AND added_by = %s', [fid, session.get('user')])
